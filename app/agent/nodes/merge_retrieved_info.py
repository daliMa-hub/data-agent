    ...
    try:
            ...
            for relevant_column in relevant_columns:
                if relevant_column not in retrieved_columns_map:
                    ...
                    if column_info is None:
                        raise ValueError(f"元数据库中未找到指标关联字段: {relevant_column}")
        ...
        for retrieved_value in retrieved_values:
            ...
            if column_id not in retrieved_columns_map:
                ...
                if column_info is None:
                    raise ValueError(f"元数据库中未找到取值关联字段: {column_id}")
        ...
        for table_id in table_to_columns_map.keys():
            # 查询主外键字段
            key_columns: list[ColumnInfo] = await meta_mysql_repository.get_key_column_info_by_table_id(table_id)
        ...
        for table_id, columns in table_to_columns_map.items():
            ...
            if table is None:
                raise ValueError(f"元数据库中未找到表信息: {table_id}")
            columns = [
